...
Design Canvas, visual overlays, and editing are separate layers. The current Design Canvas owns only navigation: viewport state, page framing, natural mouse and trackpad pan, wide safe zoom, pan recovery, fit, center, and reset. It does not mutate HTML source by itself.
