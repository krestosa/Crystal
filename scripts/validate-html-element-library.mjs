...
const validateLocalScript = packageData.scripts?.['validate:local'] ?? '';
const validateLocalRunnerIncludesHtmlElementLibrary = source.validateLocal.includes('validate:html-element-library');
...
expect(
  validateLocalScript.includes('validate:html-element-library') ||
    (validateLocalScript.includes('scripts/validate-local.mjs') && validateLocalRunnerIncludesHtmlElementLibrary),
  'validate:local does not include validate:html-element-library through package.json or scripts/validate-local.mjs.'
);
...
for (const category of mandatoryCategories) {
  expect(source.libraryConstants.includes(`id: ${quote}${category}${quote}`), `Missing mandatory category: ${category}`);
}
...
for (const state of ['none', 'no-project', 'no-preview-target', 'no-selection', 'missing-snapshot', 'stale-snapshot', 'mismatched-selection', 'ambiguous-selection', 'matched-target', 'unsupported-target']) {
  expect(source.insertionTargetTypes.includes(`| ${quote}${state}${quote}`) || source.insertionTargetSelectors.includes(`state: ${quote}${state}${quote}`), `Insertion target state missing: ${state}`);
}
