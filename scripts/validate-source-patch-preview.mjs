...
const paths = {
  ...
  runtimeCommandBus: "packages/core/commands/command-bus.ts",
  commandPreviewBusIndex: "packages/core/commands/command-preview-bus/index.ts",
  commandPreviewBusTypes: "packages/core/commands/command-preview-bus/command-preview-bus.types.ts",
  commandPreviewBusConstants: "packages/core/commands/command-preview-bus/command-preview-bus.constants.ts",
  commandPreviewBusValidators: "packages/core/commands/command-preview-bus/command-preview-bus.validators.ts",
  commandPreviewBusPreview: "packages/core/commands/command-preview-bus/command-preview-bus.preview.ts",
...
const legacyPreviewBusFolderIndex = await readOptionalText("packages/core/commands/command-bus/index.ts");
...
const commandPreviewBusSource = [
  source.commandPreviewBusIndex,
  source.commandPreviewBusTypes,
  source.commandPreviewBusConstants,
  source.commandPreviewBusValidators,
  source.commandPreviewBusPreview
].join("\n");
...
const dryRunSource = [sourcePatchSource, anchorSource, commandPreviewBusSource, plannerSource, panelSource].join("\n");
...
expect(source.runtimeCommandBus.includes("export class CommandBus"), "Legacy runtime command-bus.ts must still exist.");
expect(source.runtimeCommandBus.includes("async execute") && source.runtimeCommandBus.includes("commandBus = new CommandBus()"), "Legacy runtime CommandBus API was unexpectedly changed.");
expect(!source.runtimeCommandBus.includes("CommandPreviewResult"), "Legacy runtime command-bus.ts must not be converted into the preview bus.");
expect(!source.runtimeCommandBus.includes("COMMAND_PREVIEW_READY_STATUS"), "Legacy runtime command-bus.ts must not export preview-bus constants.");
expect(!source.runtimeCommandBus.includes("dryRun: true"), "Legacy runtime command-bus.ts must not contain dry-run preview contracts.");
expect(!legacyPreviewBusFolderIndex, "Colliding packages/core/commands/command-bus/ preview folder index must not remain.");

expectIsBarrel(source.commandPreviewBusIndex, "command-preview-bus index.ts");
for (const token of ["CommandEnvelope", "CommandPreviewResult", "CommandExecutionBlocker", "CommandBusValidationResult", "dryRun: true", "preview-ready", "blocked", "invalid", "unsupported"]) {
  expect(commandPreviewBusSource.includes(token), `Command preview bus token missing: ${token}`);
}
expect(source.commandPreviewBusPreview.includes("./command-preview-bus.constants"), "Command preview bus preview module must import command-preview-bus constants explicitly.");
expect(source.commandPreviewBusPreview.includes("./command-preview-bus.types"), "Command preview bus preview module must import command-preview-bus types explicitly.");
expect(source.commandPreviewBusValidators.includes("./command-preview-bus.types"), "Command preview bus validator must import command-preview-bus types explicitly.");
expect(source.commandPreviewBusValidators.includes("validateCommandEnvelope"), "Command preview bus validator is missing.");
expect(source.commandPreviewBusPreview.includes("previewCommandEnvelope"), "Command preview bus dry-run preview function is missing.");
expect(source.commandPreviewBusPreview.includes("executeCommandEnvelope") && source.commandPreviewBusPreview.includes("createCommandExecutionBlocker"), "Command preview bus execution blocker is missing.");
expect(commandPreviewBusSource.includes("Command execution is blocked in Phase 6B"), "Command preview bus does not explicitly block execution.");
expect(!commandPreviewBusSource.includes("from \"../command-bus\""), "Command preview bus must not import through ambiguous ../command-bus.");
expect(!commandPreviewBusSource.includes("commands/command-bus"), "Command preview bus must not import through the legacy command-bus module path.");
...
expect(source.htmlInsertionPlanner.includes("from \"../command-preview-bus\""), "HTML insertion planner must import preview contracts from command-preview-bus.");
expect(!source.htmlInsertionPlanner.includes("from \"../command-bus\""), "HTML insertion planner must not import preview contracts from ambiguous ../command-bus.");
expect(!source.htmlInsertionPlanner.includes("from \"../../commands/command-bus\""), "HTML insertion planner must not import preview contracts from commands/command-bus.");
...
expect(source.commandPreviewRenderer.includes("packages/core/commands/command-preview-bus"), "Command preview renderer must import CommandPreviewResult from command-preview-bus.");
expect(!source.commandPreviewRenderer.includes("packages/core/commands/command-bus"), "Command preview renderer must not import CommandPreviewResult from legacy command-bus.");
expect(!source.commandPreviewRenderer.includes("from \"../../../../../../../packages/core/commands/command-bus\""), "Command preview renderer must not use the ambiguous command-bus package path.");
...
for (const ambiguousImportToken of ["from \"../command-bus\"", "from \"../../commands/command-bus\"", "from \"../../../../../../../packages/core/commands/command-bus\""]) {
  expect(!dryRunSource.includes(ambiguousImportToken), `Ambiguous preview bus import found: ${ambiguousImportToken}`);
}
