...
import type { ProjectPreviewIssue, ProjectPreviewLoadResult, ProjectPreviewReloadReason, ProjectPreviewState, ProjectPreviewTarget } from "../../../../../packages/core/project/preview/project-preview.types";
...
export async function loadProjectPreview(reason: ProjectPreviewReloadReason = "manual", preferredRelativePath: string | null = null): Promise<ProjectPreviewLoadResult> {
  ...
  try {
    ...
    const pendingState = patchPreview({ rootPath: targetSelection.target.rootPath, target: targetSelection.target, previewUrl: null, status: requestedStatus, lastError: null, issues: [], issueCount: 0, lastIssueAt: null, lastReloadReason: reason, isSyncedWithProjectGraph: false });
...
function isPreviewIssueRelevantToCurrentTarget(issue: ProjectPreviewIssue, state: ProjectPreviewState): boolean {
  ...
  if (issuePath === normalizeProjectRelativeIssuePath(state.target.relativePath)) return true;
  if (state.target.directDependencyRelativePaths.some((relativePath) => normalizeProjectRelativeIssuePath(relativePath) === issuePath)) return true;
  return state.target.dependencies.some((dependency) => isPreviewIssuePathForDependency(issuePath, state.target as ProjectPreviewTarget, dependency));
}
